...
csv_file_path = 'amendment_act_updated.csv'
pdf_directory = 'amendment_act_updated'
base_url = 'https://lom.agc.gov.my/'
targeted_url = 'https://lom.agc.gov.my/principal.php?type=amendment'
...
def scrape_page():
    ...
    table = soup.find('table', {'id': 'data-amendment'})
    ...
    if table:
        for row in table.find('tbody').find_all('tr'):
            ...
            date_of_royal_assent = cells[2].text.strip()
            publication_date = cells[3].text.strip()
            date_of_commencement = cells[4].text.strip()
            
            download_links = cells[5].find_all('a')
...
with open(csv_file_path, mode='w', newline='', encoding='utf-8') as csv_file:
    writer = csv.DictWriter(csv_file, fieldnames=['Act No.', 'Titles', 'Date of Royal Assent', 'Publication Date', 'Date of Commencement'])
    writer.writeheader()  
